...
def move(cell, dir, grid)
  ...
  # if box, recursion
  if ['O', '@'].include?(grid[cell[0]][cell[1]])
    result = move([cell[0] + dir[0], cell[1] + dir[1]], dir,
                  grid)
  end
...
assert('calculates with the real input', sum_of_gps_coordinates(real_input), 1_526_018)
