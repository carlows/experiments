...
def parse(input)
  ...
  lines.map do |line|
    parts = line.match(/\[(.+)\] (\(.+\))+/)
    goal = parts[1].split('').map { |c| c == '#' ? 1 : 0 }.join('').to_i(2)
...
assert("works with the sample input", solve_part_one(input), 7)

real_input = File.read('input10.txt')
assert('works with the real input', solve_part_one(real_input), 441)
