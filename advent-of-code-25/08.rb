def part_one(input)
  ...
  map_of_points.group_by { |k, v| v }.map { |k, v| v.size }.sort { |a, b| b <=> a }.take(3).reduce(&:*)
end
...
assert('calculates all the distances', part_one(input), 20)

real_input = File.read('input08.txt')

assert('calculates all the distances for the real input', part_one(real_input), 80446)
